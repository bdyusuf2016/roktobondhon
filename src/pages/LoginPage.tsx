...
export const LoginPage: React.FC = () => {
  ...
  const { loginWithEmail, loginWithPhoneOtp, switchDemoRole } = useAuth();
  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    ...
    if (!otpSent) {
      ...
      setOtpSent(true);
      setOtp('123456'); // Pre-fill mock OTP for smooth demo testing
      return;
    }
    ...
    try {
      ...
      setError(err.message || 'ওটিপি যাচাই ব্যর্থ হয়েছে।');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-12 space-y-6">
        ...
        {mode === 'phone' ? (
          <form onSubmit={handlePhoneSubmit} className="space-y-4 text-xs">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-2.5 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg shadow-xs border border-red-700/60 transition-colors"
            >
          ...
          <form onSubmit={handleEmailSubmit} className="space-y-4 text-xs">
            <div>
              ...
              <input
                ...
                placeholder="admin@roktobondon.org"
      ...
      {/* One-Click Demo Role Switcher Section */}
