...
export const HospitalDirectoryPage: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'list'>(() => {
    try {
      const saved = localStorage.getItem('roktobondon_hospital_view_mode');
  ...
  const handleViewModeChange = (mode: 'grid' | 'list') => {
    setViewMode(mode);
    try {
      localStorage.setItem('roktobondon_hospital_view_mode', mode);
    } catch {
      // ignore
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex items-center justify-between text-xs text-slate-500 font-medium px-1 flex-wrap gap-2">
        <span>মোট <strong className="font-mono text-slate-800 text-sm">{filteredHospitals.length}</strong> টি হাসপাতাল ও ব্লাড ব্যাংক পাওয়া গেছে</span>
      ...
      {/* Hospital Cards (Grid or List View) */}
                ...
                >
                        ...
                        <div className="flex items-center gap-1">
                          {isUnverified && (
                            <button
                              ...
                              className="px-2 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 text-[10px] font-bold rounded-lg border border-emerald-200 transition-colors flex items-center gap-1 cursor-pointer"
                          ...
                          <button
                            ...
                            className="p-1.5 text-slate-500 hover:text-blue-700 hover:bg-blue-50 rounded-lg border border-slate-200 hover:border-blue-200 transition-colors cursor-pointer"
                          ...
                          <button
                            ...
                            className="p-1.5 text-slate-500 hover:text-red-700 hover:bg-red-50 rounded-lg border border-slate-200 hover:border-red-200 transition-colors cursor-pointer"
                  ...
                  <div className="pt-4 border-t border-slate-100 space-y-2.5">
                    ...
                    <button
                      ...
                      className="w-full py-2 px-3 bg-red-50 hover:bg-red-100 text-red-700 hover:text-red-800 text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 border border-red-200 transition-colors cursor-pointer"
                    >
          ...
          <div className="flex flex-col gap-3.5">
            {filteredHospitals.map((hospital) => {
              ...
              return (
                <div
                  key={hospital.id}
                  className="bg-white rounded-xl border border-slate-200/90 shadow-xs hover:border-slate-300 hover:shadow-md transition-all p-4 flex flex-col lg:flex-row lg:items-center justify-between gap-4 group"
                >
                  <div className="flex items-start gap-3.5 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <h3 className="font-bold text-slate-900 text-base leading-snug group-hover:text-red-700 transition-colors truncate">
                          {hospital.nameBn}
                        </h3>
                        <span className={`text-[10px] px-2 py-0.5 rounded-full border font-semibold ${catBadge.className}`}>
                          {catBadge.label}
                        </span>
                        {hospital.isOpen24Hours && (
                          <span className="text-[10px] px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200 font-semibold flex items-center gap-1">
                      ...
                      <div className="flex items-center gap-2 text-xs text-slate-600 flex-wrap">
                        <span className="text-slate-400 font-mono text-[11px]">{hospital.nameEn}</span>
                  ...
                  {/* Right Actions */}
                  <div className="flex items-center gap-2 flex-wrap shrink-0 justify-end border-t lg:border-t-0 pt-2.5 lg:pt-0 border-slate-100">
                    <a
                      href={`tel:${hospital.hotline}`}
                      className="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-lg flex items-center gap-1.5 shadow-xs transition-colors"
                    >
                      <PhoneCall className="w-3.5 h-3.5" />
                      <span>কল: {hospital.hotline}</span>
                    </a>
                    ...
                    {hospital.ambulancePhone && (
                      <a
                        href={`tel:${hospital.ambulancePhone}`}
                        className="px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-lg flex items-center gap-1 transition-colors border border-slate-200"
                        title="অ্যাম্বুলেন্স কল করুন"
                      >
                        <Ambulance className="w-3.5 h-3.5 text-red-600" />
                        <span className="hidden sm:inline">অ্যাম্বুলেন্স</span>
                    ...
                    {hospital.mapUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="p-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-lg flex items-center gap-1 transition-colors border border-slate-200"
                        title="গুগল ম্যাপসে দেখুন"
                      >
                        <MapPin className="w-3.5 h-3.5 text-blue-600" />
                      </a>
                    ...
                    <button
                      ...
                      className="py-1.5 px-2.5 bg-red-50 hover:bg-red-100 text-red-700 text-xs font-bold rounded-lg flex items-center gap-1 border border-red-200 transition-colors cursor-pointer"
                    >
                      <PlusCircle className="w-3.5 h-3.5" />
                      <span className="hidden sm:inline">রক্তের আবেদন</span>
                    </button>
