...
export const DonorCard: React.FC<DonorCardProps> = ({
  ...
  // Privacy rules:
  // Show phone only if donor enabled showPhone AND user is logged in, or if user is admin
  const canViewPhone =
    ...
    <>
      <div className="bg-white rounded-xl border border-slate-200/90 p-4 shadow-[0_1px_3px_rgba(0,0,0,0.04)] hover:border-slate-300 hover:shadow-md transition-all relative overflow-hidden flex flex-col justify-between">
        ...
        <div className="pt-2.5 border-t border-slate-100 flex items-center gap-2">
          ...
          {/* Call button / privacy guard */}
          {canViewPhone ? (
            isPhoneRevealed ? (
              <a
                href={`tel:${donor.phone}`}
                className="py-2 px-2.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-semibold text-xs flex items-center gap-1 border border-emerald-200 transition-colors font-mono"
                title="সরাসরি কল করুন"
              >
                <Phone className="w-3.5 h-3.5" />
                <span className="text-[11px]">{donor.phone.slice(-6)}</span>
              </a>
            ) : (
              <button
                type="button"
                onClick={() => setIsPhoneRevealed(true)}
                className="py-2 px-2.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium text-xs flex items-center gap-1 border border-slate-200 transition-colors"
                title="নম্বর দেখুন"
              >
                <Phone className="w-3.5 h-3.5 text-slate-500" />
                <span className="text-[11px]">নম্বর</span>
              </button>
            )
          ) : (
            <span
